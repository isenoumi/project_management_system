...
onMounted(() => {
  ...
  getgetId(datalist.id).then((res: any) => {
    if (res.code === 200) {
      console.log(res.data, "查询id");
      detailsList.value = res.data;
      //月份赋值
      formReport.value.list[0].getMonth = res.data.getMonth;
      //付款金额
      formReport.value.list[0].payAmount = res.data.payAmount;
      //付款日期
      formReport.value.list[0].paymentDate = res.data.paymentDate;
      //付款事由
      formReport.value.list[0].paymentRemarks = res.data.applicationNotes;
      //filrcode
      formReport.value.list[0].fileCode = res.data.fileCode;
      //
      formReport.value.list[0].cosUrl = res.data.file;
      if (res.data.file) {
        //附件上传
        let fileList = [
          {
        ...
        formReport.value.list[0].fileList = fileList;
      }
      //一级下拉
      getlevel1Subject(res.data.projectId);
      formReport.value.list[0].oneLevel = res.data.oneId;
      //二级下拉
      let data = {
        id: res.data.oneId,
      };
      getlevel2Subjects(data).then((twores: any) => {
        if (twores.code === 200) {
          formReport.value.list[0].twoSubject = twores.data;
          formReport.value.list[0].secondary = res.data.twoId;
        }
      });
      //三级下拉
      let params = {
      ...
      getlevel2Subjects(params).then((threeres: any) => {
        if (threeres.code === 200) {
          formReport.value.list[0].threeSubject = threeres.data;
          formReport.value.list[0].threeLevel = res.data.budgetSubjectId;
        }
      });
      //表格数据
      let tabdata = {
      ...
      getbudpage(tabdata).then((tableres: any) => {
        if (tableres.code === 200) {
          ...
          formReport.value.list[0].datalist = [];
          formReport.value.list[0].datalist = list;
        }
        formReport.value.list[0].monthshow = false;
      });
    }
  });
});
