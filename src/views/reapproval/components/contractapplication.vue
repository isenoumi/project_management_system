...
//结束付款审批
const stateEnd = () => {
  gettheEndExamine(query.proId).then((res: any) => {
    if (res.code === 200) {
      reverted();
      return ElMessage({
        type: "success",
...
onMounted(() => {
  ...
  if (route.query.id) {
    query.proId = route.query.id;
  }

  //付款方式枚举
  paymentMethod();
  // contractList();
  getgetId(query.proId).then((res: any) => {
    if (res.code === 200) {
      detailsList.value = res.data;
      contractList(res.data.projectId);
      //所属合同
      query.contract = res.data.contractId;
      //付款金额
      query.payment = res.data.payAmount;
      query.paymentMethod = res.data.paymentWay;
      query.paymentDate = res.data.paymentDate;
      query.paymentRemarks = res.data.applicationNotes;
      query.cosUrl = res.data.file;
      query.fileCode = res.data.fileCode;
      if (res.data.file) {
        //附件上传
        let fileList = [
          {
